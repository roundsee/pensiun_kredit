...
<div class="container" x-data="templateForm()">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="mb-0">Buat Product Template</h4>
    ...
    <form method="POST" action="{{ route('product_templates.store') }}">
        @csrf
        ...
        <div class="card mb-3">
            ...
            <div class="card-body row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="template_description" class="form-control" value="{{ old('template_description') }}">
                </div>
            </div>
        </div>
            ...
            <div class="card-body p-0">
                <table class="table table-sm mb-0">
                    <thead class="table-light"><tr>
                        <th>Field Label <small class="text-muted">(sesuai PDF)</small></th>
                        <th>Field Name <small class="text-muted">(snake_case)</small></th>
                        <th>Tipe</th>
                        <th>Wajib</th>
                        <th></th>
                    </tr></thead>
                        ...
                        <template x-for="(row, i) in debtorRows" :key="row._id">
                            <tr>
                                <td><input type="text" class="form-control form-control-sm"
                                    :name="`fields[${globalIndex(row)}][field_label]`"
                                    x-model="row.field_label"
                                    @input="row.field_name = toSnake(row.field_label)"
                                    required></td>
                                <td><input type="text" class="form-control form-control-sm"
                                    :name="`fields[${globalIndex(row)}][field_name]`"
                                    x-model="row.field_name"></td>
                                <td>
                        ...
                        <template x-for="(row, i) in financialRows" :key="row._id">
                            <tr>
                                ...
                                <td><input type="number" step="0.0001" class="form-control form-control-sm"
                                    :name="`fields[${globalIndex(row)}][default_value]`"
                                    x-model="row.default_value" placeholder="0"></td>
...
function templateForm() {
    ...
    return {
        ...
        allRows() {
            return [...this.debtorRows, ...this.submissionRows, ...this.financialRows];
        },

        globalIndex(row) {
            return this.allRows().findIndex(r => r._id === row._id);
        },
